...
class StandFunc:
    ...
    def recover(self, push, push_threshold, gain1, gain2):
        ...
        move = [0,0]
        max_response = [gain1,gain2] #[gain1,gain2] # max responses of shoulders, hips, respectively
    ...
    def get_angles(self, z_lean, l_deriv, l_ddot, shoulder_hip_diffs):
        # use ankle or hip/shoulder ctrl
        if shoulder_hip_diffs[0] + shoulder_hip_diffs[1] > 0:
            diff = 0
            diff2 = shoulder_hip_diffs[1] # hip difference (negative for "reverse" hip strategy/UVC)
            diff3 = shoulder_hip_diffs[0] # shoulder difference
        else:
            # ankle controller gains
            Kpa = 0.0005 # proportional gain for ankles    
        ...
        f1 = self.init_angles[14] - diff # right sagittal ankle
        f2 = self.init_angles[15] + diff # left sagittal ankle
        self.init_angles[14] = f1
        self.init_angles[15] = f2
        
        cmds=np.zeros(6) # initialize output array
        cmds[0] = f[0] - diff3 # right sagittal shoulder
        cmds[1] = f[1] + diff3 # left sagittal shoulder
        cmds[2] = f[10] - diff2 # right sagittal hip
        cmds[3] = f[11] + diff2 # left sagittal hip
        cmds[4] = f1 # right sagittal ankle
        cmds[5] = f2 # left sagittal ankle
...
class Stand:
    ...
    def __init__(self,tobe):
        ...
        self.responding=False # denotes when robot is responding quickly to disturbance
        self.going_home=False # denotes when robot is returning to home position after disturbance response
        self.reflex=[0,0] # difference bet. reflex and home joint positions when responding to disturbance
    ...
    def _update_acceleration(self, msg):
        ...
        if ((acc > self.push_threshold) and (self.going_home == False)):
    ...
    def _do_stand(self):
        ...
        while not rospy.is_shutdown() and self.standing: # control loop  
                ...
                if self.home_time <= 0: # end of reflex period
                    ...
                    if self.home_time > (pause_at_home_time + return_time): # during reflex response
                        ...
                        if self.home_time > pause_at_home_time: # returning to home position
                            ...
                            reflex_return = [reflex_return1, reflex_return2]                            
            
            if self.responding == True:
                self.reflex = func.recover(self.push,self.push_threshold,self.var1,self.var2)  
            ...
            response_in_radians=func.get_angles(self.lean,self.l_deriv,self.l_ddot,reflex_return)        
